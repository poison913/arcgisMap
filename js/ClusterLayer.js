...
function initClusterLayer() {
    dojo.declare("ClusterLayer", esri.layers.GraphicsLayer, {
        constructor: function (options) {
            ...
            this._sr = options.spatialReference || new esri.SpatialReference({ "wkid": 4326 });
        ...
        onClick: function (e) {
            ...
            if (singles.length > this._maxSingles) {
                ...
                if (singles.length == 1) {
                    ...
                    e.stopPropagation();
                    showinfo(singles[0].attributes.projectId)
                   
                }
                else {
                    var multiPoint = new esri.geometry.Multipoint(map.spatialReference);
                    for (var i = 0; i < singles.length; i++) {
                        multiPoint.addPoint(singles[i]);
                    }
                    map.setExtent(multiPoint.getExtent().expand(3));
                }
            }
        },
        ...
        _clusterAddPoint: function (p, cluster) {
            ...
            x = (parseFloat(p.x) + parseFloat(cluster.x * count)) / (count + 1);
            y = (parseFloat(p.y) + parseFloat(cluster.y * count)) / (count + 1);
            cluster.x = x;
            cluster.y = y;
            ...
            // increment the count
            cluster.attributes.clusterCount = cluster.attributes.clusterCount + 1;
        ...
        _showCluster: function (c) {
            ...
            if (c.attributes.clusterCount == 1) {
                ...
                //        var text = attr["attributes"]["weight"] == undefined ? attr["attributes"]["leftNeedAmount"] : attr["attributes"]["weight"];

                //        var textback = new esri.symbol.PictureMarkerSymbol('images/textback.png', text.length * 8, 24);
                //        textback.setOffset(text.length * 4 + 16, 5);
                //        var graphic_back = new esri.Graphic(point, textback);
                //        this.add(graphic_back);

                //        var font = new esri.symbol.Font("12px").setDecoration("none");
                //        var label = new esri.symbol.TextSymbol(text, font)
                //          .setColor(new dojo.Color([255,255,255]))
                //          .setOffset(text.length * 4 + 16, 0);
                //        this.add(
                //          new esri.Graphic(
                //            point,
                //            label,
            ...
            var label = new esri.symbol.TextSymbol(c.attributes.clusterCount, font)
        ...
        _updateLabel: function (c) {
            ...
            if (label.length == 1) {
                ...
                this.remove(label[0]);
                var font = new esri.symbol.Font().setDecoration("none");
                var newLabel = new esri.symbol.TextSymbol(c.attributes.clusterCount, font)
                  .setColor(new dojo.Color(this._clusterLabelColor))
                  .setOffset(0, this._clusterLabelOffset);
                this.add(
                  new esri.Graphic(
