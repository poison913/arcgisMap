...
function INIT() {
    dojo.declare("HeatmapLayer", esri.layers.DynamicMapServiceLayer, {
        ...
        constructor: function (properties) {
            //lang.mixin(this.properties, properties);
            ...
            //lang.mixin(this.config, properties.config);
            ...
            dojo.connect(this._map, 'resize', this.resizeHeatmap);
        ...
        convertHeatmapData: function (parsedData) {
            ...
            //var screenUtils = new esri.geometry.screenUtils;
            // if data
            if (parsedData.data) {
                // for all x values
                    ...
                    if (parsedData.data.hasOwnProperty(xParsed)) {
                        ...
                        for (yParsed in parsedData.data[xParsed]) {
                            if (parsedData.data[xParsed].hasOwnProperty(yParsed)) {
                                ...
                                screenGeometry = this.toScreenGeometry(this._map.extent, this._map.width, this._map.height, parsedData.data[xParsed][yParsed].dataPoint);
                                //screenGeometry = this._map.toScreen(new esri.geometry.Point(parsedData.data[xParsed][yParsed].dataPoint.x, parsedData.data[xParsed][yParsed].dataPoint.y, map.spatialReference));
        ...
        toScreenGeometry: function (extent, width, height, dataPoint) {
            var maxLon = extent.xmax;
            var minLon = extent.xmin;
            var maxLat = extent.ymax;
            var minLat = extent.ymin;
            var scaleX = (maxLon - minLon) / width;
            var scaleY = (maxLat - minLat) / height;
            var data = {};
            data.x = (dataPoint.x - minLon) / scaleX;
            data.y = (dataPoint.y - minLat) / scaleY;
            return data;
        },
        ...
        parseHeatmapData: function (features) {
            ...
            if (features) {
                ...
                for (i = 0; i < features.length; i++) {
                    ...
                    dataPoint = new esri.geometry.Point(features[i].geometry);
                    // check point
